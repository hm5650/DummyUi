...
local UIUpdateLoopRunning = false
SaveTheme = {}
...
function Library:watchConfig(configTable, uiElements)
    -- Store the reference to UI elements for this config
    ConfigWatchers[configTable] = {
        elements = uiElements,
        lastValues = {}
    }
    
    -- Initialize last values
    for elementType, elementTable in pairs(uiElements) do
        if elementType ~= "Button" then  -- Skip buttons
            for elementName, elementObject in pairs(elementTable) do
                if configTable[elementName] ~= nil then
                    ConfigWatchers[configTable].lastValues[elementName] = configTable[elementName]
                end
            end
        end
    end
    ...
    if not UIUpdateLoopRunning then
        startUIUpdateLoop()
    end
end

-- Function to start the UI update loop
function startUIUpdateLoop()
    if UIUpdateLoopRunning then return end
    
    UIUpdateLoopRunning = true
    
    game:GetService("RunService").Heartbeat:Connect(function()
        for configTable, watcherData in pairs(ConfigWatchers) do
            for elementType, elementTable in pairs(watcherData.elements) do
                if elementType ~= "Button" then  -- Skip buttons
                    for elementName, elementObject in pairs(elementTable) do
                        local currentValue = configTable[elementName]
                        local lastValue = watcherData.lastValues[elementName]
                        
                        -- Check if value has changed
                        if currentValue ~= lastValue then
                            -- Update the UI element
                            updateUIElement(elementObject, elementType, currentValue)
                            
                            -- Update last value
                            watcherData.lastValues[elementName] = currentValue
                        end
                    end
                end
            end
...
function updateUIElement(elementObject, elementType, newValue)
    if elementType == "Toggle" and elementObject.SetValue then
        elementObject:SetValue(newValue)
    elseif elementType == "Slider" and elementObject.SetValue then
        elementObject:SetValue(newValue)
    elseif elementType == "Dropdown" and elementObject.SetValue then
        elementObject:SetValue(newValue)
    elseif elementType == "Keybind" and elementObject.SetKey then
        elementObject:SetKey(newValue)
    elseif elementType == "Textbox" and elementObject.SetValue then
        elementObject:SetValue(newValue)
    elseif elementType == "ColorPicker" and elementObject.SetValue then
        elementObject:SetValue(newValue)
...
function Library:Window(p)
    ...
    local ConfigTable = p.Config or {}  -- Store the config table
    ...
    function Tabs:Tab(p)
        ...
        function Func:Toggle(p)
            ...
            local Title = p.Title or 'null'
            local Desc = p.Desc or ''
            local Name = p.Name or ''  -- Config tracking name
            ...
            local function change()
                ...
                -- Update config if name is provided
                if Name ~= '' and ConfigTable[Name] ~= nil then
            ...
            -- Track this element for config updates if name is provided
            if Name ~= '' and ConfigTable[Name] ~= nil then
                WindowUI:trackElement("Toggle", Name, New)
            end
        ...
        function Func:Button(p)
            ...
            Click.MouseButton1Click:Connect(function()
                ...
                -- Update config if name is provided (for one-time triggers)
                if Name ~= '' and ConfigTable[Name] ~= nil then
                    ConfigTable[Name] = true
                    task.wait(0.1)
                    ConfigTable[Name] = false
                end
            end)
            
            -- Track button for one-time trigger updates if name is provided
            if Name ~= '' and ConfigTable[Name] ~= nil then
                local ButtonObj = {}
                function ButtonObj:Trigger()
                    Click.MouseButton1Click:Fire()
                end
                WindowUI:trackElement("Button", Name, ButtonObj)
            end
        end
        ...
        function Func:Slider(p)
            ...
            -- Track this element for config updates if name is provided
            if Name ~= '' and ConfigTable[Name] ~= nil then
                WindowUI:trackElement("Slider", Name, New)
            end
        ...
        function Func:Dropdown(p)
            ...
            -- Track this element for config updates if name is provided
            if Name ~= '' and ConfigTable[Name] ~= nil then
                WindowUI:trackElement("Dropdown", Name, New)
            end
        ...
        function Func:Keybind(p)
            ...
            -- Track this element for config updates if name is provided
            if Name ~= '' and ConfigTable[Name] ~= nil then
                WindowUI:trackElement("Keybind", Name, New)
            end
        ...
        function Func:ColorPicker(p)
            ...
            local function UpdateColorPicker(nope)
                ...
                if lastColor ~= Picker_1.BackgroundColor3 then
                    ...
                    -- Update config if name is provided
                    if Name ~= '' and ConfigTable[Name] ~= nil then
                        ConfigTable[Name] = {R = math.floor(r), G = math.floor(g), B = math.floor(b)}
                    end
                end
            end
            ...
            -- Track this element for config updates if name is provided
            if Name ~= '' and ConfigTable[Name] ~= nil then
                WindowUI:trackElement("ColorPicker", Name, New)
            end
        ...
        function Func:Textbox(p)
            ...
            local function o()
                if #TextLabel_1.Text > 0 then
                    ...
                    -- Update config if name is provided
                    if Name ~= '' and ConfigTable[Name] ~= nil then
                        ConfigTable[Name] = TextLabel_1.Text
                    end
                end
            end
            ...
            -- Track this element for config updates if name is provided
            if Name ~= '' and ConfigTable[Name] ~= nil then
                WindowUI:trackElement("Textbox", Name, New)
            end
    ...
    -- Create WindowUI object for config tracking
    local WindowUI = {
        ConfigElements = {},  -- Store UI elements by type
        ConfigTable = ConfigTable
    }
    
    -- Add helper function to track UI elements
    function WindowUI:trackElement(elementType, elementName, elementObject)
        if not self.ConfigElements[elementType] then
            self.ConfigElements[elementType] = {}
        end
        self.ConfigElements[elementType][elementName] = elementObject
    end
    
    -- Add UI updater functionality
    function WindowUI:updateFromConfig()
        for elementType, elementTable in pairs(self.ConfigElements) do
            if elementType ~= "Button" then  -- Skip buttons
                for elementName, elementObject in pairs(elementTable) do
                    local configValue = self.ConfigTable[elementName]
                    if configValue ~= nil then
                        -- Update the UI element based on its type
                        if elementType == "Toggle" and elementObject.SetValue then
                            elementObject:SetValue(configValue)
                        elseif elementType == "Slider" and elementObject.SetValue then
                            elementObject:SetValue(configValue)
                        elseif elementType == "Dropdown" and elementObject.SetValue then
                            elementObject:SetValue(configValue)
                        elseif elementType == "Keybind" and elementObject.SetKey then
                            elementObject:SetKey(configValue)
                        elseif elementType == "Textbox" and elementObject.SetValue then
                            elementObject:SetValue(configValue)
                        elseif elementType == "ColorPicker" and elementObject.SetValue then
                            elementObject:SetValue(configValue)
    ...
    -- Start a loop to monitor config changes
    local lastConfigValues = {}
    ...
    task.spawn(function()
        while ScreenGui and ScreenGui.Parent do
            ...
            for key, value in pairs(ConfigTable) do
                if lastConfigValues[key] ~= value then
                    lastConfigValues[key] = value
                    
                    -- Find and update the corresponding UI element
                    for elementType, elementTable in pairs(WindowUI.ConfigElements) do
                        if elementTable[key] and elementType ~= "Button" then
                            local elementObject = elementTable[key]
    ...
    -- Add the UI updater to the global library watchers
    table.insert(Library._windowUIs or {}, WindowUI)
    Library._windowUIs = Library._windowUIs or {}
    
    -- Return both Tabs and the WindowUI for advanced control
    local ReturnTable = {}
    for k, v in pairs(Tabs) do
        ReturnTable[k] = v
    end
    ReturnTable.UI = WindowUI
    
    return ReturnTable
end

function Library:updateUIFromConfig(configTable)
    local watcherData = ConfigWatchers[configTable]
    if watcherData then
        for elementType, elementTable in pairs(watcherData.elements) do
