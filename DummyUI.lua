...
-- Add this after the themes table and before the do block
local UILoop = {
    Elements = {},
    Running = false,
    Connection = nil
    ...
    self.Connection = game:GetService("RunService").Heartbeat:Connect(function()
        for _, element in pairs(self.Elements) do
            if element.Type == "Toggle" then
                local currentValue = element.GetValue()
                if currentValue ~= element.LastValue then
            ...
            elseif element.Type == "Keybind" then
                ...
                local currentKey = element.GetKey()
                if currentKey ~= element.LastKey then
                    element.SetKey(currentKey)
                    element.LastKey = currentKey
                end
...
function UILoop:Register(element)
    table.insert(self.Elements, element)
end

function UILoop:Unregister(element)
...
function Library:Window(p)
	...
	function Tabs:Tab(p)
        ...
        -- Store internal value for UILoop access
        ...
        -- UILoop accessor functions
        function New:GetValue()
            return internalValue
        end
        
        function New:SetValueProgrammatic(t)
            internalValue = t
            Value = not t
            change()
        end
        
        -- Update internal value when toggle changes
        ...
        -- Register with UILoop
        UILoop:Register({
            Type = "Toggle",
            GetValue = function() return internalValue end,
            SetValue = function(v) New:SetValueProgrammatic(v) end,
        ...
        -- Store internal value for UILoop
        local internalValue = Value
        ...
        -- Update internal value when slider changes
        local originalUpdateSlider = updateSlider
        updateSlider = function(value)
            originalUpdateSlider(value)
            internalValue = Value
        end
		...
		function Func:Dropdown(p)
         ...
         -- Update internal value when dropdown changes
		...
		function Func:Textbox(p)
         ...
         -- Update internal value when textbox changes
         local originalCallback = Callback
         Callback = function(value)
             internalValue = value
             originalCallback(value)
         ...
         TextLabel_1:GetPropertyChangedSignal("Text"):Connect(function()
             if TextLabel_1:IsFocused() then
                 internalValue = TextLabel_1.Text
             end
         end)
   ...
   Library.UILoop = UILoop
	return Tabs
end

-- Add this to the end of the library, before the return statement:
function Library:StartUILoop()
    UILoop:Start()
end

function Library:StopUILoop()
    UILoop:Stop()
end

-- Also update the main Library table to include UILoop
Library.UILoop = UILoop
